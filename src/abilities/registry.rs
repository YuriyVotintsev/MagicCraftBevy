...
impl ActionRegistry {
    ...
    pub fn register(&mut self, handler: Box<dyn ActionHandler>) -> ActionTypeId {
        let name = handler.name().to_string();
        let id = ActionTypeId(self.handlers.len() as u32);
        self.name_to_id.insert(name.clone(), id);
        self.id_to_name.push(name);
        self.handlers.push(handler);
        id
    ...
    pub fn get_id(&self, name: &str) -> Option<ActionTypeId> {
        self.name_to_id.get(name).copied()
    }

    #[allow(dead_code)]
    pub fn get_name(&self, id: ActionTypeId) -> Option<&str> {
        self.id_to_name.get(id.0 as usize).map(|s| s.as_str())
    }

    pub fn get(&self, id: ActionTypeId) -> Option<&dyn ActionHandler> {
        self.handlers.get(id.0 as usize).map(|h| h.as_ref())
    }
