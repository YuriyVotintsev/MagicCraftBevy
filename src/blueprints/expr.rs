...
impl EntityExpr {
    ...
    pub fn eval(&self, source: &SpawnSource) -> Option<Entity> {
        match self {
...
impl ScalarExpr {
    pub fn eval(&self, source: &SpawnSource, stats: &ComputedStats) -> f32 {
        match self {
            Self::Literal(v) => *v,
...
impl VecExpr {
    pub fn eval(&self, source: &SpawnSource, stats: &ComputedStats) -> Vec2 {
        match self {
