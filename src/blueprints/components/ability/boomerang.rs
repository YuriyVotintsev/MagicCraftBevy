...
fn boomerang_system(
    ...
    for (entity, attached, boomerang, mut state, transform, mut velocity) in &mut query {
        if !state.returning {
            let distance = crate::coord::to_2d(transform.translation).distance(boomerang.spawn_position);
        ...
        if state.returning {
            if let Ok(owner_transform) = owner_query.get(attached.owner) {
                ...
                let direction = crate::coord::to_2d(to_owner).normalize();
                velocity.0 = crate::coord::ground_vel(direction * boomerang.speed);
            } else {
                commands.entity(entity).despawn();
            }
        }
