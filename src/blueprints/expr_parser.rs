...
impl Parser {
    ...
    fn parse_ident_expr(&mut self, name: &str) -> Result<TypedExpr, String> {
        match name {
            ...
            "stat" => self.parse_stat_call(),
            "index" => Ok(TypedExpr::Scalar(ScalarExprRaw::Index)),
            "count" => Ok(TypedExpr::Scalar(ScalarExprRaw::Count)),
            "normalize" => self.parse_normalize(),
            "rotate" => self.parse_rotate(),
            "lerp" => self.parse_lerp(),
            "vec2" => self.parse_vec2_constructor(),
            "from_angle" => self.parse_from_angle(),
