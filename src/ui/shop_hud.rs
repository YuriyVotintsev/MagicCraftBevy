...
use crate::rune::{roll_shop_offer, Dragging, RerollState, RuneGrid, ShopOffer};
...
pub fn register(app: &mut App) {
    app.add_systems(OnEnter(WavePhase::Shop), spawn_shop_hud)
        .add_systems(
            Update,
            (start_run_system, reroll_button_system, update_reroll_label)
                .run_if(in_state(WavePhase::Shop)),
        );
}

pub fn spawn_shop_hud(mut commands: Commands) {
    let root = commands
        .spawn((
