...
fn parse_provided_fields(attrs: &[Attribute]) -> Option<Vec<String>> {
    for attr in attrs {
        if attr.path().is_ident("provided_fields") {
            if let Ok(Meta::List(list)) = attr.meta.clone().try_into() as Result<Meta, _> {
                let tokens = list.tokens.to_string();
    ...
    if let Type::Path(type_path) = ty {
        if let Some(segment) = type_path.path.segments.last() {
            if segment.ident == "Option" {
                if let PathArguments::AngleBracketed(args) = &segment.arguments {
                    if let Some(GenericArgument::Type(inner_ty)) = args.args.first() {
                        return !is_scalar_expr_type(inner_ty) && !is_vec_expr_type(inner_ty);
                    }
                }
            }
        }
...
fn parse_component_name(attrs: &[Attribute]) -> Option<syn::Ident> {
    for attr in attrs {
...
#[proc_macro_derive(GenerateRaw, attributes(activator, raw, default_expr, entities, provided_fields, component))]
pub fn derive_generate_raw(input: TokenStream) -> TokenStream {
    ...
    let provided_fields = parse_provided_fields(&input.attrs);
    let component_name = parse_component_name(&input.attrs);

    match &input.data {
        Data::Struct(data_struct) => {
            match &data_struct.fields {
                Fields::Named(fields) => {
                    generate_for_named_struct(name, &raw_name, fields, is_activator, provided_fields, component_name)
                }
                Fields::Unit => {
                    generate_for_unit_struct(name, &raw_name, is_activator, component_name)
                }
                Fields::Unnamed(_) => {
                    syn::Error::new(
                        input.span(),
...
fn get_component_type(ty: &Type) -> Option<TokenStream2> {
    if is_scalar_expr_type(ty) {
        Some(quote! { f32 })
    } else if is_vec_expr_type(ty) {
        Some(quote! { bevy::prelude::Vec2 })
    } else if is_entity_expr_type(ty) {
        Some(quote! { bevy::prelude::Entity })
    } else if is_option_scalar_expr(ty) {
        Some(quote! { Option<f32> })
    } else if is_option_vec_expr(ty) {
        Some(quote! { Option<bevy::prelude::Vec2> })
    } else {
        None
    }
}
...
fn generate_for_named_struct(
    ...
    component_name: Option<syn::Ident>,
) -> TokenStream {
    ...
    for field in &fields.named {
        ...
        } else if is_option_vec_expr(field_ty) {
            if let Some(ref default_str) = default_expr {
                raw_fields.push(quote! {
                    #[serde(default)]
                    pub #field_name: Option<crate::abilities::expr::VecExprRaw>
                ...
                resolve_fields.push(quote! {
                    #field_name: Some(self.#field_name.as_ref()
                        .map(|v| v.resolve(stat_registry))
                        .unwrap_or_else(|| crate::abilities::expr::parse_and_resolve_vec(#default_str, stat_registry)))
                });
                ...
                required_fields_code.push(quote! {
                    if let Some(ref expr) = self.#field_name {
                        fields = fields.union(expr.required_fields());
                    }
                });
            }
        ...
        if let Some(comp_ty) = get_component_type(field_ty) {
            component_fields.push(quote! {
                pub #field_name: #comp_ty
            });
            let eval_code = get_eval_code(field_name, field_ty).unwrap();
            insert_fields.push(quote! {
                #field_name: #eval_code
            });
        } else if is_vec_entity_def(field_ty) {
            component_fields.push(quote! {
                pub #field_name: Vec<crate::abilities::entity_def::EntityDef>
            });
            insert_fields.push(quote! {
    ...
    let component_and_insert = if let Some(comp_name) = component_name {
        quote! {
    ...
    let output = quote! {
        ...
        pub fn required_fields_and_nested(raw: &#raw_name) -> (crate::abilities::context::ProvidedFields, Option<(crate::abilities::context::ProvidedFields, &[crate::abilities::entity_def::EntityDefRaw])>) {
...
fn generate_for_unit_struct(
    ...
    let output = quote! {
        ...
        pub fn required_fields_and_nested(_raw: &#raw_name) -> (crate::abilities::context::ProvidedFields, Option<(crate::abilities::context::ProvidedFields, &[crate::abilities::entity_def::EntityDefRaw])>) {
